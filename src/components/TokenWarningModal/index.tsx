          ...
          {chainId && (
            <ExternalLink style={{ fontWeight: 400 }} href={getEtherscanLink(chainId, token.address, 'token')}>
              <TYPE.blue title={token.address}>
                {shortenAddress(token.address)} (View on {process.env.REACT_APP_CHAIN_SYMBOL})
              </TYPE.blue>
            </ExternalLink>
          )}
        </AutoColumn>
      ...
      <WarningContainer className="token-warning-container">
        <AutoColumn gap="lg">
          ...
          <TYPE.body color={'red2'}>
            Anyone can create an Xrc20 token on Alyx Chain with <em>any</em> name, including creating fake versions of
            existing tokens and tokens that claim to represent projects that do not have a token.
          </TYPE.body>
          <TYPE.body color={'red2'}>
            This interface can load arbitrary tokens by token addresses. Please take extra caution and do your research
            when interacting with arbitrary Xrc20 tokens.
          </TYPE.body>
          <TYPE.body color={'red2'}>
